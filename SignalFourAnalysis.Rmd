...
rf.tune.mu = tune.randomForest(as.factor(INCAPACITATING_FLAG) ~ TOTAL_NUMBER_OF_VEHICLES + 
    ...
    S4_AGING_DRIVER_COUNT + S4_UNRESTRAINED_COUNT, data=crashtrain_balanced, mtry=c(3), nodesize=c(17), ntree=c(100,200,300,400,500,600))
summary(rf.tune.mu)
rf.tune.mu$best.model
```
...
modelrf = randomForest(as.factor(INCAPACITATING_FLAG)~., crashtrain_balanced, nodesize=17, mtry=3, ntree=300, proximity=T, importance=T)
...
modelrfstep = randomForest(as.factor(INCAPACITATING_FLAG)~TOTAL_NUMBER_OF_VEHICLES + 
    ...
    S4_IS_DRUG_RELATED + S4_IS_HIT_AND_RUN + S4_IS_INTERSECTION_RELATED + 
    S4_IS_LANE_DEPARTURE_RELATED + S4_MOTORCYCLE_COUNT + S4_MOPED_COUNT + 
    S4_AGING_DRIVER_COUNT + S4_UNRESTRAINED_COUNT, crashtrain_balanced, nodesize=17, mtry=3, ntree=300,proximity=T, importance=T)
...
modelgbm = gbm(INCAPACITATING_FLAG~., data=crashtrain_balanced, distribution = 'bernoulli', n.trees = 5000, shrinkage = 0.01, interaction.depth = 2, cv.folds = 10)
summary(modelgbm)
